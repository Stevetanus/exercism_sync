...
def resistor_label(colors):
    ...
    multiplier_index = 3 if len(colors) > 4 else 2
    tolerance_index = 4 if len(colors) > 4 else 3 
    band_value = 0
    band_value_str = ""
    # get band value
    ...
    band_value_str = get_ohms_str(float(band_value))
    # add tolerance
    tolerance_band = colors[tolerance_index]
    band_value_str += " ±" + color_tolerance_dict[tolerance_band]

    return band_value_str
